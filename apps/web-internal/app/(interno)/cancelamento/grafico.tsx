 ...
 * └───────────────────────────────────────────────────────────────────────────┘
 */
import { type MesDoGrafico } from '@pulse/success'
...
export function Grafico({ meses, janela }: { meses: readonly MesDoGrafico[]; janela: 6 | 12 }) {
  ...
  const teto = Math.max(1, ...meses.map((m) => Number(m.totalCentavos)))
    ...
    <Card title="Cancelamento por mês">
        ...
        {meses.map((m) => {
          ...
          return (
            <div key={m.mes} className="flex h-full min-w-0 flex-1 flex-col items-center gap-1">
              <span className="text-nota tabular-nums text-ink-3">
                {m.maduro && total > 0 ? BRL(m.totalCentavos) : ''}
              </span>
              ...
              <div className="flex w-full flex-1 items-end justify-center gap-0.5">
                ...
                >
                  <div
                    ...
                    style={{ height: `${m.maduro ? (total / teto) * 100 : 100}%` }}
                  >
                    {!m.maduro ? (
                      ...
                      <>
                        {apurado < total && (
                          <div
                            ...
                            style={{ height: `${((total - apurado) / total) * 100}%` }}
      ...
      <ul className="mt-4 flex flex-wrap gap-x-4 gap-y-1.5">
        ...
        <li className="flex items-center gap-1.5 text-nota text-ink-3">
          <span className="h-2.5 w-2.5 shrink-0 rounded-sm border border-dashed border-line-strong" />
          Em apuração
        </li>
      </ul>
